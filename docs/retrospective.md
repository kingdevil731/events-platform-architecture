...
read cost. The rewrite keeps the ledger as truth and adds a maintained balance
...
before assets or maintenance. Client-generated IDs with server-authoritative
ordering so an offline device with a wrong clock still sequences correctly.
Decimal for all money and quantity, never float. A correctness suite over
inventory and cash arithmetic that must pass before any customer touches it.
